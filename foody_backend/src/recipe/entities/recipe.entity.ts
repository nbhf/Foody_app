...
export class Recipe extends TimestampEntites{
    ...
    @ManyToMany(() => User, user => user.createdRecipes) // Assurez-vous que l'entité User a une relation `createdRecipes`
    createdBy: User;

    @ManyToMany(() => User, user => user.savedRecipes) // Assurez-vous que l'entité User a une relation `createdRecipes`
    savedBy: User;
}
